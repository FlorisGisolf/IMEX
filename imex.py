...
#        def load_meta_tab(event):
#            print('hi')
#            print(self.meta_load)
#            if self.meta_load == 0 and len(self.im_list) > 0:                
#                self.meta_load = 1
#                print(len(self.im_list))
#                print(self.meta_load)
#                self.meta_images = []
#                meta_keys = [36867,36868,271,272,34853,37510,270,256,40962,257,40963, 315]
#                taglist = []                
#                keylist=['filename','Date and time','Camera','Image width','Image heigth','UserComment','ImageDescription','Artist']
#                #self.meta_data = {key: [] for key in keylist}
#                self.meta_data={}
#                for i in range(0,len(meta_keys)):
#                    taglist.append(TAGS.get(meta_keys[i],meta_keys[i]))
#
#                #### list with exif tags ####
#                # 36867 and/or 36868 create date and time
#                # 271 and 272 camera make and model Exif.Image.Model	
#                # 315 artist
#                # image name
#                # 34853 GPS
#                # 37510 UserComment
#                # 270 ImageDescription
#                # 256 or 40962 ImageWidth ExifImageWidth
#                # 257 or 40963 ImageLength
#
...
#                        self.meta_images.append(ImageTk.PhotoImage(load))
#                        meta_test = {}
#                        if info:
#                            self.meta_data[z]['filename'] = self.im_list[z]
#                            
#                            for q in range(0,len(meta_keys)):
#                                try:
#                                    meta_test[taglist[q]] =  info[meta_keys[q]]
#                                except KeyError:
#                                    meta_test[taglist[q]] = ''
#                            # set artist
#                            self.meta_data[z]['Artist'] = meta_test[taglist[11]]
#                            self.meta_data[z]['UserComment'] = meta_test[taglist[5]]
#                            self.meta_data[z]['ImageDescription'] = meta_test[taglist[6]]
#                            # set date and time
#                            if meta_test[taglist[0]] == '':
#                                self.meta_data[z]['Date and time'] = meta_test[taglist[1]]
#                            else:
#                                self.meta_data[z]['Date and time'] = meta_test[taglist[0]]
#                            # set camera make and model
#                            if meta_test[taglist[3]] == '':
#                                self.meta_data[z]['Camera'] = meta_test[taglist[2]]
#                            else:
#                                self.meta_data[z]['Camera'] = meta_test[taglist[3]]
#                            # set image width
#                            if meta_test[taglist[7]] == '':
#                                self.meta_data[z]['Image width'] = meta_test[taglist[8]]
#                            else:
#                                self.meta_data[z]['Image width'] = meta_test[taglist[7]]
#                            # set image height
#                            if meta_test[taglist[9]] == '':
#                                self.meta_data[z]['Image heigth'] = meta_test[taglist[9]]
#                            else:
#                                self.meta_data[z]['Image heigth'] = meta_test[taglist[10]]
#
#                            
#                        else:
#                            self.meta_data[z]['filename'] = self.im_list[z]
#                            self.meta_data[z]['Date and time'] = ''
#                            self.meta_data[z]['Camera'] = ''
#                            self.meta_data[z]['Image width'] = ''
#                            self.meta_data[z]['Image heigth'] = ''
#                            self.meta_data[z]['UserComment'] = ''
#                            self.meta_data[z]['ImageDescription'] = ''
#                            self.meta_data[z]['Artist'] = ''
#                            
#                        #self.meta_data.append(meta_test)
##                tableYscrollbar = Scrollbar(width = 16) #scroll  bar for canvas
##                self.tTable = TableCanvas(self.tab2,data=self.meta_data, editable=False,cellbackgr='#555555',foreground='#FFFFFF',scrollregion=(0, 0, 0, 500), yscrollcommand=tableYscrollbar.set,width=500, height=5)
##                self.tTable.show()
##                tableYscrollbar.config(command=self.tTable.yview)
##                tableYscrollbar.place(in_=self.tTable,relx=1.0, relheight=1)
##                self.tTable.bind('<Enter>', self._bound_to_mousewheel)
##                self.tTable.bind('<Leave>', self._unbound_to_mousewheel)
#                self.tree =ttk.Treeview(self.tab2,columns=('','','','','','','',''))
#                self.tree.place(x=20,y=20)
#                self.tree.heading('#0',text='File name')
#                self.tree.heading('#1',text='Date and time')
#                self.tree.heading('#2',text='Artist')
#                self.tree.heading('#3',text='Camera')
#                self.tree.heading('#4',text='User Comment')
#                self.tree.heading('#5',text='Image width')
#                self.tree.heading('#6',text='Image height')
#                self.tree.heading('#7',text='Image description')
#                
#                testdict = {'1':[1,2,3,4],'test':['a','b','c','d']}
#                for ii in range(0,len(self.meta_data)):    
#                    self.tree.insert('','end', text=self.meta_data[ii]['filename'],value=(self.meta_data[ii]['Date and time'],self.meta_data[ii]['Artist'],self.meta_data[ii]['Camera'],self.meta_data[ii]['UserComment'],self.meta_data[ii]['Image width'],self.meta_data[ii]['Image heigth'],self.meta_data[ii]['ImageDescription']))
#        self.notebook.bind("<<NotebookTabChanged>>", load_meta_tab)
